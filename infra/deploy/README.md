...
- `infra/deploy/nginx/templates/*.template`: roteamento HTTP/HTTPS
- `infra/scripts/bootstrap_ubuntu_2404.sh`: instala Docker e Compose no Ubuntu 24.04
- `infra/scripts/deploy.sh`: faz `git pull` e `docker compose up -d --build`
- `infra/scripts/init_letsencrypt.sh`: emite o primeiro certificado
- `infra/scripts/renew_letsencrypt.sh`: renova certificado depois
...
cp infra/deploy/env/monarch.env.example infra/deploy/env/monarch.env
cp infra/deploy/env/monarch-core.env.example infra/deploy/env/monarch-core.env
cp infra/deploy/env/whatsapp.env.example infra/deploy/env/whatsapp.env
```
...
3. Rodar `infra/scripts/bootstrap_ubuntu_2404.sh`.
4. Preencher os arquivos `infra/deploy/.env`, `infra/deploy/env/monarch.env`, `infra/deploy/env/monarch-core.env` e `infra/deploy/env/whatsapp.env`.
5. Rodar `infra/scripts/deploy.sh`.
6. Rodar `infra/scripts/init_letsencrypt.sh`.
